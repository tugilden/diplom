"""
Алгоритм Мегиддо для 2D линейного программирования (O(n)).

Классический алгоритм Мегиддо (1984) — первый детерминированный O(n) алгоритм
для 2D LP. Основная идея:

1. Поворот координат: максимизировать u = (p*x + q*y)/||c||
2. Ограничения: u <= α_i*v + β_i (верхние) и u >= α_j*v + β_j (нижние)
3. f(v) = min_i(α_i*v + β_i) — нижняя огибающая верхних прямых (вогнута)
4. g(v) = max_j(α_j*v + β_j) — верхняя огибающая нижних прямых (выпукла)
5. Находим v, максимизирующее f(v) при g(v) <= f(v)

Принцип prune-and-search для линейной сложности:
- Находим медиану v_med точек пересечения пар прямых
- Оцениваем f(v_med) и g(v_med)
- Если g(v_med) > f(v_med) (недопустимо):
  * Находим активные прямые: active_up (дают f) и active_low (дают g)
  * Сравниваем медианные наклоны: если slope_low > slope_up, идём влево, иначе вправо
  * Отбрасываем ≥ n/2 прямых
- Если g(v_med) <= f(v_med) (допустимо):
  * Смотрим наклон активной верхней прямой
  * Если > 0, идём вправо (f возрастает), иначе влево
  * Отбрасываем ≥ n/2 прямых

Сложность: T(n) = T(n/2) + O(n) = O(n)
"""
...
def solve_megiddo(constraints: List[Tuple[float, float, float]], p: float, q: float) -> Tuple[Optional[float], Optional[float], Optional[float], str]:
    ...
    def megiddo_rec(up_list, low_list, depth=0):
        ...
        if depth > 500:  # Защита от бесконечной рекурсии
            return brute_force(up_list, low_list)
        
        # Шаг 1: Находим точки пересечения пар прямых
        crosses = []
        ...
        v_med = sorted(crosses)[len(crosses) // 2]
        
        # Шаг 3: Оцениваем f(v_med) и g(v_med)
        f_val = f_of_v(v_med, up_list)
        g_val = g_of_v(v_med, low_list)
        
        if g_val > f_val + EPS:
            # Шаг 4a: v_med недопустимо (g > f)
            ...
            if active_low and active_up:
                sl = sorted([a for a, b in active_low])[len(active_low) // 2]
                su = sorted([a for a, b in active_up])[len(active_up) // 2]
                
                if sl > su + EPS:
                    # g растёт быстрее f -> слева g может стать < f
                    # Отбрасываем верхние с a < su: слева они ниже активной -> f станет меньше, g > f останется
                    new_up = [(a, b) for a, b in up_list if a >= su - EPS]
                    new_low = list(low_list)
                else:
                    # f растёт быстрее g -> справа f может стать > g
                    # Отбрасываем верхние с a > su: справа они выше активной -> f станет меньше, g > f останется
                    new_up = [(a, b) for a, b in up_list if a <= su + EPS]
                    new_low = list(low_list)
            ...
            # Шаг 4b: v_med допустимо (g <= f)
            active_up = [(a, b) for a, b in up_list if abs(a * v_med + b - f_val) < EPS]
            
            if active_up:
                median_slope = sorted([a for a, b in active_up])[len(active_up) // 2]
                
                if median_slope > EPS:
                    # f возрастает -> максимум справа
                    # Отбрасываем верхние с a > median_slope: справа они выше активной -> f станет меньше
                    ...
                    # Отбрасываем верхние с a < median_slope: слева они выше активной -> f станет меньше
                    new_up = [(a, b) for a, b in up_list if a >= median_slope - EPS]
                    new_low = list(low_list)
                    for item in active_up:
                        if item not in new_up:
...
def run_tests():
    print("=" * 60)
    print("ТЕСТЫ: Мегиддо vs Симплекс")
    print("=" * 60)
...
def main():
    import sys
    if len(sys.argv) < 2:
        print("python megiddo_simplex.py test|<файл>")
        return
    ...
    if status == 'optimal':
        print(f"\nМегиддо: x={x:.6f}, y={y:.6f}, max={val:.6f}")
